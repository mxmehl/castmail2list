...
- From header:
  - `From` is the list's configured `from_addr_override` if set, otherwise the list address.
...
:---|:---:|:---:
From | `ml.from_addr_override` or `ml.address` | `"Sender Name via List Name <list@address>"`
