...
pytest: ## Run pytest: unit tests
	$(POETRY) run pytest --cov=$(APPDIR)

pylint: ## Run pylint: static code analysis
	$(POETRY) run pylint --disable=fixme $(APPDIR)

formatting: ## Run isort and black: format code
	$(POETRY) run isort --check $(APPDIR)
...
translations-update: ## Update translation template and .po files
	@$(POETRY) run pybabel extract -F $(APPDIR)/babel.cfg -o $(APPDIR)/messages.pot $(APPDIR)
	@$(POETRY) run pybabel update -i $(APPDIR)/messages.pot -d $(APPDIR)/translations

translations-compile: ## Compile .po files to .mo files
	@$(POETRY) run pybabel compile -d $(APPDIR)/translations
