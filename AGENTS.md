...
Flask-based mailing-list service: polls IMAP mailboxes, stores messages in a SQLAlchemy-backed database, and serves a web UI with bounce handling. See [README.md](../README.md) and [CONTRIBUTING.md](../CONTRIBUTING.md).
...
mise run test-all                  # all checks at once
mise run fix-all                   # auto-fix lint/format issues
mise run translations-compile      # compile Babel translations
```

Run debug server: `uv run castmail2list-cli --config config.yaml --debug` (copy `config.example.yaml` → `config.yaml` first and fill in required keys).
...
Feature docs:
* [modes & headers](../doc/modes_and_headers.md)
* [sender authorization](../doc/sender_authorization.md)
* [API](../doc/api.md)
...
- **Translations:** Wrap user-visible strings with `_()`. After adding strings: `mise run translations-update` then `mise run translations-compile`.
...
All steps must pass. Fix failures; do not disable tests. To reproduce a CI matrix failure locally: `uv python install <version> && uv sync`, then re-run the failing command.
